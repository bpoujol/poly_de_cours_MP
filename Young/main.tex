...
\begin{encart}[Mesure du diamètre de Alpha Orionis par perte de cohérence spatiale]
	...
	Dans l'expérience des trous d'Young, connaissant l'écart des trous $a$ à partir duquel on a brouillage des interférences, on peut directement remonter à la largeur anglulaire de la source, et donc à une estimation de la largeur de la source.

	Le même protocole a été éxécuté en 1921 par Michelson et Pease pour mesurer le diamètre de l'étoile Alpha Orionis. En guise de trous d'Young, ils ont sélectionné deux rayons lumineux issus de l'étoile à l'aide de deux miroirs séparés d'une distance $a$ de plusieurs mètres, qu'ils ont ensuite recombinés pour observer les franges d'Young. Il est alors possible de déterminer l'écartement entre les miroirs pour lequel on observe le brouillage des interférences pour retrouver la largeur angulaire de la source. La distance $d$ entre l'étoile et le système solaire étant connue par des méthodes de parallaxe, on en déduit facilement la taille de l'étoile.
...
Dans ce cours, nous nous limiterons exclusivement à un réseau dit en transimission (la lumière traverse le réseau). Par ailleurs, nous allons supposer que le réseau est constitué d'un grand nombre $N \gg 1$ de trous similaires aux trous d'Young.
...
\begin{proposition}
	Soit un réseau contenant $N$ motifs. Si on numérote les motifs dans l'ordre, en un point d'observation, les phases $(\varphi_i)_{i \in \llbracket 1,N \rrbracket}$ des rayons diffractés par chacun des motifs forment une suite arithmétique de raison $\delta$ :
	\begin{equation}
		\varphi_{i+1} = \varphi + \delta
	\end{equation}
\end{proposition}
...
Pour le savoir, il faut calculer précisément l'intensité lorsque l'on a non plus deux ondes qui interfèrent, mais $N$ ondes avec une phase en progression arithmétique. On va adopter la modélisation complexe, en supposant que les $N$ ondes ont même amplitude :
\begin{equation}
	s_i = s_0 \cos(2 \pi \nu t - i \Delta \varphi)
\end{equation}
...
\begin{figure}[htbp]
	\centering \includegraphics[width=\linewidth]{reseau_courbe.png}
...
On constate que la largeur des pics est d'autant plus fine que le réseau contient un grand nombre de traits : un réseau va petre sensible à des variations de phase de l'ordre de $1/N$, c'est-à-dire à des différences de marche de l'ordre de $\lambda/N$. Pour un réseau typique, on pourra avoir $N$ de l'ordre de $10^3$ ou plus : le réseau permet donc d'avoir une précision redoutable par rapport aux interférences à deux ondes.
