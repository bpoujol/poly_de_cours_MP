...
\chapterimage{sunset.png} % Table of contents heading image
...
\begin{capex}
	On considère une antenne constituée d'un fil de longueur $a$, parcourue par un courant uniforme $i(t) = I_0 \cos(\omega t)$. En utilisant la conservation de la charge, montrer que l'antenne est équivalente à un dipôle oscillant dont on calculera le moment dipolaire $\vv p(t)$.
\end{capex}
...
\begin{figure}[htbp]
	\centering \includegraphics[width=0.35\linewidth]{coordonnees.png}
	\caption{Système de coordonnées utilisé pour étudier le rayonnement dipolaire; Source : poly de cours de Valérie Heinrich.}
\end{figure}
...
\begin{figure}[htbp]
	\centering \includegraphics[width=0.6\linewidth]{lignes_de_champ.png}
	\caption{Lignes de champ électrique associées au rayonnement dipolaire. Source : poly de cours de Olivier Granier.}
\end{figure}
...
\begin{figure}[t]
	\centering \includegraphics[width=\linewidth]{rayonnement_antennes.png}
	\caption{Diagramme de rayonnement pour des antennes de différente taille, représentés en 3D. Source : université de Carthage.}
\end{figure}

\begin{encart}[Diagramme de rayonnement des antennes]
	Ici, nous avons supposé que le courant $i$ est uniforme dans l'antenne ce qui permet de l'assimiler à un dipôle oscillant. Mais en réalité, on choisit plutôt das antennes dont la longueur est de l'ordre de la longueur d'onde électromagnétique étudiée ce qui permet d'augmenter la puissance d'émission. Les calculs précédents ne sont alors plus valides et on obtient des diagrammes d'émission plus complexes, comme représentés ci dessous, qui dépendent du rapport entre la longueur $L$ de l'antenne et la longueur d'onde $\lambda$ émise.
\end{encart}
...
\begin{figure}[htbp]
	%\centering \includegraphics[width=0.4\linewidth]{schema_diffusion.png}
	\centering \includegraphics[width=0.9\linewidth]{principe_diffusion.png}
	\caption{Schéma de principe de la diffusion de la lumière par les molécules de l'atmosphère. Sources : Wikimedia Commons et poly de cours de Olivier Granier.}
\end{figure}
...
\begin{figure}[htbp]
	\centering \includegraphics[width=0.5\linewidth]{thomson.png}
	\caption{Modèle de Thomson de l'atome. Source : Encyclopaedia Britannica.}
\end{figure}
...
Nous pouvons identifier différents domaines sur cette courbe :
\begin{itemize}
	\item Le domaine de Rayleigh pour $\omega \ll \omega_0$, c'est dans ce domaine que se trouve la diffusion de la lumière par les molécules de l'atmosphère. Dans ce domaine, on a $\mathcal{P}_{\rm diff} \propto \omega^4$, donc une forte dépendance de la puissance diffusée à la longueur d'onde ;
	\item La résonance pour $\omega \approx \omega_0$ ;
	\item Et le domaine de Thomson pour $\omega \gg \omega_0$, qui concerne plutôt les grosses particules diffusantes comme les gouttelettes de nuage. Dans ce domaine, on a $\mathcal{P}_{\rm diff}$ indépendant de $\omega$ et toutes les longueurs d'onde sont diffusées avec la même puissance.
\end{itemize}

\begin{figure}[t]
	\centering \includegraphics[width=\linewidth]{ciel_bleu.png}
	\caption{Diffusion de la lumière par les molécules du ciel. Source : Poly de cours de Olivier Granier.}
\end{figure}

\begin{encart}[Le bleu du ciel]
	La couleur du ciel vient directement de la diffusion de la lumière. Pour les molécules de diazote et de dioxygène, la pulsation de résonance $\omega_0$ est élevée, si bien que la lumière visible se trouve dans le domaine de Rayleigh, ainsi la diffusion des courtes longueurs d'onde est dominante par rapport à celle des grandes longueurs d'onde. Ceci explique la couleur bleue du ciel.
	Au coucher de soleil, l'essentiel de la lumière directe a été diffusée si bien que seules les grandes longueurs d'onde peuvent arriver jusqu'à nos yeux, le soleil apparaît donc rouge-orangé au coucher de soleil. Cet effet est exacerbé lorsque l'atmosphère contient beaucoup de molécules diffusantes, typiquement lors d'un épisode de brumes sahariennes : la diffusion étant très forte, le ciel est d'un bleu très pâle et lumineux dans la journée, et le coucher de soleil est très rouge. Au contraire, en haute montagne, le ciel est d'un bleu profond ce qui indique que moins de particules diffusantes sont présentes dans l'atmosphère.

	La diffusion de la lumière est également une source de polarisation, en effet, nous avons vu qu'aucun rayonnement n'est diffusé dans la direction du dipôle. Or le dipôle se trouve dans la même direction que le champ électrique incident, donc perpendiculairement à la direction de propagation du champ électrique incident ce qui donne deux directions possibles. Si la diffusion se fait à un angle de 90$^\circ$, l'une de ces deux directions diffuse une puissance nulle, et le rayonnement diffusé est polarisé linéairement, ce qui se voit à l'aide d'un polariseur.
\end{encart}

\begin{figure}[htb]
	\centering \includegraphics[width=0.8\linewidth]{ciel_pola_1.png}
	\centering \includegraphics[width=0.5\linewidth]{ciel_pola_2.png}
	\caption{Polarisation de la lumière diffusée par les molécules d'air dans l'atmosphère. Source : Poly de cours de Valérie Heinrich.}
\end{figure}

\paragraph{Section efficace (hors programme)}
...
\begin{definition}[Section efficace de diffusion]
