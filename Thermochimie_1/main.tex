...
\begin{theorem}[Premier principe de la thermodynamique (rappel)]
    Soit un système fermé $\Sigma$ subissant une transformation thermodynamique. Nous notons $U$ son énergie interne, $E_c$ son énergie cinétique macroscopique, et   $E_p$ son énergie potentielle macroscopique. Nous notons également $Q$ l'énergie thermique reçue par le système au cours de la transformation, et $W$ le travail des forces extérieures sur le système pendant la transformation. Alors :
    \begin{equation}
        \Delta(U+E_p+E_c) = W + Q
    \end{equation}
\end{theorem}
...
Nous avons vu en première année qu'il est utile de définir une nouvelle variable, appelée enthalpie :
\begin{definition}[Enthalpie (rappel)]
    L'enthalpie d'un système d'énergie interne $U$, de volume $V$ et à la pression $p$ est donnée par :
    \begin{equation}
        H = U + p V
    \end{equation}
\end{definition}
...
\begin{capex}
En appliquant le premier principe, établir l'expression de la variation d'enthalpie au cours d'une transformation monobare.
\end{capex}
...
%\begin{equation}
%    \Delta H = \Delta U + p_{finale} V_{final} - p_{initiale} V_{initial} = \Delta U + p_{ext} \Delta V
%\end{equation}
%
...
Cette relation est très utile pour décrire les réactions chimiques. En effet, elle relie directement les transferts thermiques lors de la réaction aux transferts thermiques entre le système et son environnement. Par ailleurs, la quantité $\Delta H$ se décompose en deux parties :
\begin{itemize}
    \item Une contribution $\Delta H_{\rm chimique}$ issue de la rupture des liaisons des réactifs et de la formation de nouvelles liaisons. Il s'agit de la variation d'énergie potentielle microscopique du système ;
    \item Une contribution $\Delta H_{\rm thermique}$ issue de la variation de température du système. Il s'agit de la variation d'énergie cinétique microscopique du système correspondant à l'agitation thermique de ses consitutants.    
\end{itemize}

L'énergie chimique libérée par la réaction est donnée par la quantité $\Delta H_{\rm chimique}$. Cette énergie est ensuite convertie en énergie thermique qui peut soit rester à l'intérieur du système et l'échauffer ou le refoidir ($\Delta H_{\rm thermique}$) ou bien être transférée à l'extérieur du système ($Q$). 

Ceci nous amène à définir la quantité $\Delta H_{\rm chimique}$ de façon plus rigoureuse, puisqu'elle est la mesure de l'énergie pouvant être libérée par une réaction chimique. Il s'agit de la variation d'enthalpie du système chimique en l'absence de variation de pression ou de température. Pour définir cette quantité de manière claire, nous devons adopter certaines conventions.
...
\begin{definition}[Etat standard d'un soluté]
   Pour un composé en solution, l'état standard est l’état hypothétique de ce soluté dans le solvant lorsqu’il est inﬁniment dilué, à la pression $P \degree$ et à la concentration $C \degree = 1$\,mol$\cdot$L$^{-1}$. 
\end{definition}
\begin{remark}
...
\begin{definition}[Enthalpie standard de réaction]
    L'enthalpie standard d'une réaction chimique est la variation d’enthalpie de la réaction théorique isotherme au cours de laquelle les réactifs en nombre de moles égaux aux coeﬃcients stœchiométriques sont pris dans leur état standard, et se transforment entièrement en produits pris dans leur état standard. Elle se note $\Delta_r H \degree(T)$ et dépend de la température. Son unité est le kJ$\cdot$mol$^{-1}$.
...
Dans le cas d'une réaction à l'état standard et isotherme, on aura ainsi $\Delta H = \Delta_r H \degree$. Or on a vu que $\Delta H = Q$, autrement dit l'enthalpie de réaction correspond à l'énergie thermique reçue par le système au cours d'une transformation isotherme. Ceci nous fournit une interprétation concrète de l'enthalpie standard de réaction : 
\begin{proposition}
...
Comme la variation des fonctions d'état ne dépend pas du chemin suivi, on peut imaginer la transformation comme une succession de deux états :
\begin{itemize}
...
\begin{capex}
Déterminer la température de flamme (température atteinte par le mélange réactionnel) en supposant la réaction adiabatique.\\
\textit{Données :} : 
\begin{itemize}
\item Enthalpie de la réaction : $\Delta_r H\degree = -2\,$MJ$\cdot$mol$^{-1}$
\item Caapcités thermiques molaires : $C_{p,\ce{H2O}} = 36$\,J$\cdot$K$\cdot$mol$^{-1}$ et $C_{p,\ce{CO2}} = 37$\,J$\cdot$K$\cdot$mol$^{-1}$ 
\end{itemize}
\end{capex}
...
De manière générale, la couleur de la flamme renseigne directement sur sa température, et permettra donc de discriminer entre différentes réactions chimiques.
...
\begin{capex}
Relier le transfert thermique $Q$ consommé lors du changement d'état à l'enthalpie molaire de vaporisation de l'eau $L_{\rm vap}$.
\end{capex}
...
\begin{capex}
La réaction de photosynthèse permet de produire le glucose $\ce{C6H12O6}$ à partir de dioxyde de carbone et d'eau. 
\begin{itemize}
\item Calculer l'enthalpie standard de la réaction.
\item Expliquer pourquoi des photons sont nécessaires à la réaction. 
\item La chlorophylle absorbe des photons dans le vert. Combien de photons sont nécessaires pour produire une mole de glucose ?
\end{itemize}
\textit{Données :} $\Delta_f H^\circ(\ce{C6H12O6}) = -1273$\,kJ$\cdot$mol$^{-1}$ ;\\$\Delta_f H^\circ(\ce{H2O(l)}) = -286$\,kJ$\cdot$mol$^{-1}$ ; $\Delta_f H^\circ(\ce{CO2(g)}) = -394$\,kJ$\cdot$mol$^{-1}$  
\end{capex}

\begin{encart}[Fluorescence chlorophyllienne]
En réalité, le nombre de photons absorbés lors de la réaction de photosynthèse est plus élevé que le nombre théorique : jusqu'à trois fois plus de photons sont utilisés par une plante que le nombre calculé précédemment. L'énergie excédentaire est en partie convertie sous forme d'énergie thermique lors du processus, ce qui réchauffe la plante, et peut aussi provoquer l'évaporation de l'eau et le dessèchement.

Pour éviter ces conséquences néfastes, la plante restitue une partie de cet excès d'énergie sous forme lumineuse, par fluorescence. Les photons émis par la plante sont moins énergétiques et ont donc une longueur d'onde plus élevée, dans le rouge et dans l'infrarouge. La mission satellitaire FLEX (Fluorescence Explorer), lancée le 15 septembre 2026, a pour objectif de mesurer cette fluorescence pour en déduire l'activité photosynthétique des plantes. Ceci permet notamment d'évaluer la santé des écosystèmes, d'anticiper les situations de stress hydrique précédant les sécheresses, et d'étudier l'évolution de l'activité des plantes dans un climat qui se réchauffe et dans lequel la concentration en dioxyde de carbone augmente.

Pour cela, le satellite est équipé de l'instrumetn FLORIS, un spectromètre à réseau qui permet de séparer les différentes longueurs d'onde du spectre visible, et de sélectionner celles qui correspondent à la fluorescence des plantes.
\begin{center}
\includegraphics[width=0.45\linewidth]{photosynthese.pdf}
\includegraphics[width=0.45\linewidth]{spectre.jpeg} \\
Ci-dessus : processus de photosynthèse et spectre de la lumière émise par fluorescence.
\end{center}
