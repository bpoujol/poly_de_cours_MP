...
\chapterimage{cristaux_electrodes.png} % Table of contents heading image
...
\begin{figure}[htbp]
	\centering \includegraphics[width=0.8\linewidth]{pile_daniell.png}
...
\subsection{Bilan électrique}

\subsubsection{Bilan électrique d'une pile}
...
\subsubsection{Bialn électrique d'une électrolyse}

Il est également possible de réaliser le procédé inverse : il s'agit de l'électrolyse. Une réaction chimique non spontanée est forcée en imposant une différence de potentiel entre deux électrodes suffisante pour que la réaction chimique puisse se faire spontanément. En pratique, le générateur force les électrons à aller dans le sens opposé du sens dans lequel ils iraient spontanément. C'est le cas, par exemple, de l'électrolyse de l'eau qui sert à former le dihydrogène :
\begin{equation}
...
\begin{figure}[htbp]
	\centering \includegraphics[width=0.8\linewidth]{electrolyse_eau.png}
	\caption{Electrolyse de l'eau. Source : theory.labster.com.}
\end{figure}

\begin{capex}
	Ecrire les demi-équations électroniques se produisant à la cathode et à l'anode. \`{A} l'aide d'un bilan de quantité de matière, relier la durée $\Delta t$ de l'électrolyse au courant imposé pour réaliser l'électrolyse, noté $i$, et à la masse de dihydrogène théorique produite par électrolyse $m_{\ce{H2}, \rm th}$.
\end{capex}

\begin{definition}
	On appelle rendement faradique d'une électrolyse le rapport entre la masse théoriquement produite si tous les électrons servaient à former le produit, et la masse effectivement produite expérimentalement :
	\begin{equation}
		\eta_F = \frac{m_{\rm produit,exp}}{m_{\rm produit,th}} 
	\end{equation}
\end{definition}

\begin{remark}
	En pratique, le rendement faradique est souvent inférieur à 1 car des réactions d'oxydation ou de réduction parasites peuvent se produire au niveau de l'électrode, impliquant des ions présents en solution ou les métaux constituants des électrodes.
\end{remark}

\subsubsection{Les accumulateurs}

Il est possible de faire fonctionner un système à la fois en pile et en électrolyseur : c'est ce que l'on appelle un accumulateur. On peut alors brancher les deux électrodes sur un circuit consommateur d'énergie (on décharge alors l'accumulateur), ou bien on les branche sur un générateur et on récupère alors de l'énergie en rechargeant l'accumulateur.

\begin{figure}[t]
	\centering \includegraphics[width=0.8\linewidth]{li_ion.jpg}
	\caption{Schéma simplifié d'une batterie lithium-ion. Source : C.Schultz (2016). \textit{Anti-aging for batteries? LC-MS method development for battery electrolytes.} Shimadzu News.}
\end{figure}

\begin{encart}[Les batteries lithium-ion]
	La plupart des batteries présentes aujourd'hui dans les appareils électroniques sont des batteries lithium-ion. Elles sont constituées de deux électrodes en graphite $\ce{LiC6}$ et en oxyde de cobalt $\ce{LiCoO2}$, qui sont enrichis en lithium. Les électrodes sont séparées par un électrolyte qui conduit les ions lithium $\ce{Li^+}$ et les deux demi-équations se produisant lors de la phase de recharge de la batterie sont :
	\begin{align}
		&\ce{LiCoO2 = CoO2 + Li^+ + e^-} \\
		&\ce{C6 + Li^+ + e^- = LiC6}
	\end{align}
	Lors de la décharge, les réactions inverses se font spontanément et le courant électrique qui en résulte peut servir à alimenter un appareil électrique.
\end{encart}
...
\begin{equation}
	E = E_{+} - E_{-} = \frac{1}{n \mathcal{F}} \sum_i \nu_i \left( \mu_i^\circ(T) + R T \ln(a_i) \right) = \frac{1}{n \mathcal{F}} \left( \sum_i \nu_i \mu_i^\circ(T) + \ln \left( \prod_i a_i^{\nu_i} \right) \right)
\end{equation}
...
\end{align}
où les potentiels standard $E^\circ(\ce{Cu^{2+}/Cu})$ et $E^\circ(\ce{Zn^{2+}/Zn})$ sont des constantes.
\begin{remark}
	...
	\begin{equation}
		E^\circ(\ce{H+(aq)/H2(g)}) = 0\,\mathrm{V}
	\end{equation}
\end{remark}
...
\begin{capex}
	A partir de la formule de Nernst, retrouver l'expression entre l'enthalpie libre de réaction et les potentiels standard des couples dans le cas de la pile Daniell. En déduire une relation entre l'enthalpie libre standard de réaction et les potentiels standard. Exprimer la constante de réaction en fonction des potentiels standard des couples mis en jeu.
\end{capex}
...
\end{align}
Ces deux relations nous permettent de définir des diagrammes de prédominance ou d'existence de différentes espèces en fonction du potentiel ou du pH : les acides sont favorisés pour les pH faibles, les bases pour les pH élevés. De même, les oxydants sont favorisés aux potentiels élevés, et les réducteurs aux potentiels faibles.
...
\begin{capex}
	On donne les $pK_s$ et potentiels standard des couples intervenant dans le diagramme du fer :
...
\begin{figure}[htbp]
	\centering \includegraphics[width=0.54\linewidth]{diagramme_H2O.png}
...
\end{align}
Les courbes associées ont les équations suivantes :
\begin{equation}
	\begin{dcases}
		E(\ce{H^+ / H2}) - -0.06 pH \\
		E(\ce{O2 / H2O}) = 1.23 - 0.06pH
	\end{dcases}
\end{equation}
Ce diagramme est toujours à prendre en compte en solution aqueuse, car l'eau y est toujours présente. Le dioxygène est une espèce présente car elle se dissout naturellement dans l'eau, à moins que la solution ne soit protégée de l'atmosphère.
...
Les diagrammes potentiel-pH et le concept de pile nous permettent d'interpréter les phénomènes de corrosion, qui posent problème pour un certain nombre de dispositifs. 
...
\begin{figure}[htbp]
	\centering \includegraphics[width=0.45\linewidth]{E_pH_Al.png} \includegraphics[width=0.54\linewidth]{oxyde_aluminium.jpg}
...
\begin{figure}[p]
	\centering \includegraphics[width=0.85\linewidth]{rouille_microscope_guyane.png} \includegraphics[width=0.85\linewidth]{rouille_guyane_2.png}
