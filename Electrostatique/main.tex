...
\begin{figure}[h]
\centering\includegraphics[height=0.27\textwidth]{Pictures/charges_atome.png} \includegraphics[height=0.27\textwidth]{Pictures/ions_cristal.png}
...
$\varepsilon_0 = 8.85 \cdot 10^{-12}$\,F$\cdot$m$^{-1}$ est la permittivité diélectrique du vide.\\ 
...
\begin{definition}[Champ électrique]
    Le champ électrique, noté $\vv E$, indique en tout point la force électrostatique subie par une particule ponctuelle immobile de charge unité. Il se mesure en Volts par mètre (V$\cdot$m$^{-1}$). Une particule de charge $q$ se trouvant dans un champ électrique $\vv E$ subit alors une force électrostatique $\vv F = q \vv E$. 
\end{definition}
...
\begin{definition}[Lignes de champ]
    Une ligne de champ électrique est une ligne tangente en tout point au vecteur champ électrique $\vv E$. Les lignes de champ sont orientées dans le sens du champ électrique.
\end{definition}
...
\begin{capex}
Tracer les lignes de champ produites par :
\begin{itemize}
\item Une unique charge négative
\item Un ensemble de deux charges positives
\end{itemize}
\end{capex}
...
\begin{capex}
Calculer le travail issu du déplacement d'une particule $M$ de charge $q$ sur un chemin $AB$, au voisinage d'une particule charge $q_0$ située sur l'origine $O$.
\end{capex}

%\begin{example}
%    Calculons le travail  La particule $M$ est soumise à la force électrostatique de Coulomb et on a:
%    \begin{align}
%        W &= \int_\Gamma q \vv E \cdot \vv{\dd \ell} \\
...
\begin{capex}
Dans les deux situations de la capcité exigible précédente, tracer également l'allure des équipotentielles.
\end{capex}
...
\begin{encart}[Analogie entre potentiel électrostatique et topographie]
    ...
    \centering \includegraphics[width=0.9\textwidth]{Pictures/isolignes.png}
...
Afin de simplifier les calculs, qui sont relativement complexes, on fera ce que l'on appelle l'approximation dipolaire :
\begin{definition}
...
\begin{capex}
Exprimer le potentiel électrostatique au point $M$ en fonction de la charge $q$ et des distances $r_+$ et $r_-$, puis en fonction de $q,r$ et $\theta$.
\end{capex}
...
On constate que le potentiel $V(M)$ ne dépend que du produit entre la charge portée par les éléments du dipôle et la distance entre ces éléments; on appelle cette quantité le moment dipolaire.
...
\begin{capex}
A partir de l'expression du potentiel $V(M)$, déterminer le champ électrique $\vv E(M)$ produit par le dipôle.

On donne l'expression du gradient en coordonnées sphériques :
\begin{equation}
...
%    \grad = \frac{\partial}{\partial r} \vv{e_r} + \frac{1}{r} \frac{\partial}{\partial \theta}  \vv{e_\theta} + \frac{1}{r \sin \theta} \frac{\partial}{\partial \varphi}  \vv{e_\varphi}
%\end{equation}
%Ici le potentiel ne dépend que de $\theta$ et de $r$ et le champ $\vv E$ est donc donné par :
%\begin{align}
%    \vv E &= - \grad \left( 
...
\begin{proposition}
Le champ électrique créé par un dipôle électrostatique $\vv p = p \vv{e_z}$ placé à l'origine d'un repère de coordonnées sphériques d'axe $Oz$, est donné par :
\begin{equation}
\vv E = \frac{p}{4 \pi \varepsilon_0 r^3} \left( 2 \cos \theta \, \vv{e_r} + \sin \theta \, \vv{e_\theta}  \right)
\end{equation}
\end{proposition}
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[height=0.45\linewidth]{Pictures/champ_dipole_pres.png} \includegraphics[height=0.45\linewidth]{Pictures/champ_dipole_loin.png}
...
\begin{capex}
Dans le cas où le dipôle est soumis à un champ uniforme, calculer la résultante des forces électriques agissant sur le dipôle ainsi que leur moment au point $O$.
\end{capex}
...
Dans les conducteurs, les charges sont libres de se déplacer. Comme les charges de même signe se repoussent, elles ne vont pas rester à l'intérieur du conducteur mais vont s'accumuler à la surface du conducteur (nous étudierons ce phénomène plus en détail dans le cadre du cours d'électromagnétisme). On peut alors observer une accumulation de charges sur la surface du conducteur. Ceci va nous permettre de définir une densité surfacique de charges $\sigma$ :
...
\begin{capex}
Considérons un fil correspondant à un cylindre de rayon $a$, avec une densité volumique de charges uniforme $\rho$. Pour simplifier le modèle, on peut supposer que le rayon $a$ est petit. Alors on modélise le fil par un fil infiniment fin portant une densité linéique de charge $\lambda$.

Déterminer la densité linéique $\lambda$ équivalente du fil.
\end{capex} 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.49\linewidth]{Pictures/plan_symétrie.png} \includegraphics[width=0.49\linewidth]{Pictures/plan_antisymétrie.png}
...
Nous allons maintenant utiliser ces propriétés pour en déduire des informations sur la direction du champ électrique dans différentes configurations classiques, comme représenté sur la figure \ref{fig:symetries}. 
...
En combinant les propriétés de symétrie et d'invariance, on obtient la forme générale du champ électrique pour différentes distributions de charges. Nous allons à nouveau reprendre l'exemple des trois figures ci-dessus.

\begin{example}
Dans le cas de la figure de gauche, la distribution de charges est invariante par translation selon l'axe $(Oz)$, donc le champ électrique est indépendant de $z$. Nous avons par ailleurs vu que ce champ est perpendiculaire à $\vv{e_z}$, il est donc porté par les vecteurs $\vv{e_x}$ et $\vv{e_y}$, ainsi :
\begin{equation}
\vv E (M) = E_x(x,y) \vv{e_x} + E_y(x,y) \vv{e_y}
\end{equation}
\end{example}

\begin{capex}
Déterminer la forme général du champ électrique dans un système de coordonnées approprié, pour les figures du centre et de droite.
\end{capex}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[height=0.35\textwidth]{Pictures/flux_sphere.png} \includegraphics[height=0.35\textwidth]{Pictures/element_de_surface.png}
...
\begin{capex}
On considère une charge ponctuelle $q$ placée à l'origine d'un repère sphérique. Déterminer le flux du champ électrique à travers la sphère de centre $O$ et de rayon $r$.
\end{capex}
...
\begin{equation}
    ||\vv{E_{in}}|| S_{in} = ||\vv{E_{out}}|| S_{out}
\end{equation}
...
\begin{capex}
En appliquant le théorème de Gauss à une sphère de rayon $r$, déterminer l'expression du champ électrique produit par la sphère. On prendra soin de distinguer les cas $r>R$ et $r<R$.
\end{capex}
...
\begin{capex}
A partir d'une analyse des symétries du problème, montrer qu'en tout point $M$ de l'espace, le champ électrique prend la forme $\vv E(M) = E(z) \vv{e_z}$. Déterminer la parité de la fonction $E(z)$.
\end{capex}
...
\begin{capex}
En appliquant le théorème de Gauss à cette surface, déterminer l'expression de $E(z)$.
\end{capex}
...
\begin{capex}
Déterminer la tension $u$ aux bornes du condensateur (c'est-à-dire, la différence de potentiel entre les armatures). En déduire l'expression de la capacité $C$ du condensateur.
\end{capex} 
