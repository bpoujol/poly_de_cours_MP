...
Jusqu'à cette époque, la description de la matière en physique se faisait de manière exclusivement continue, à partir de variables telles que la masse volumique $\rho$, la densité volumique de charge $\rho$, la densité volumique de courant $\vv j$, la température $T$, la pression... On connaissait l'existence des atomes, mais on cherchait à leur donner une escription continue (pour plus de détails, consulter le DM d'électromagnétisme à ce sujet). Cependant, on commence à découvrir que dans les atomes, la charge n'est pas uniformément répartie, mais elle est concentrée dans une petite partie de l'espace :
\begin{itemize}
	\item En 1897, Thomson découvre l'électron blablabla. On découvre alors que la masse de l'électron et sa charge sont toujours les mêmes : il s'agit d'une propriété fondamentale.
	\item Puis en 1911, Rutherford découvre le noyau des atomes, à partir de la déviation de particules $\alpha$ par des noyaux d'Hélium.
\end{itemize}
...
Les réelles interrogations arrivent avec d'autres résultats. Un des échecs de la physique classique, bien connu à l'époque, est son incapacité à expliquer le rayonnement produit par un corps chauffé. Comme nous l'avons vu plus tôt dans l'année, plus un corps est chaud, plus il émet un rayonnement intense, à une longueur d'onde petite. Cependant, la physique classique est incapable d'expliquer la présence de ce pic et prédit un rayonnement qui contient une énergie infinie aux faibles longueurs d'onde. C'est ce qu'on appelle la \textit{catastrophe ultraviolette}. En 1900, Planck fait une hypothèse saugrenue : il introduit une constante $h$ dans ces calculs telle que l'énergie du rayonnement lumineux est quantifiée d'une manière proportionnelle à la fréquence : $E = n h \nu$. Son but était de faire le calcul par une autre méthode, puis de faire tendre $h$ vers 0. Or, il constate que le résultat qu'il obtient, la loi de Planck, ajuste parfaitement les observations... mais sans faire tendre $h$ vers 0 ! Il obtient la loi de Planck donnant le spectre en intensité d'un corps noir chauffé :
\begin{equation}
	I_\nu(\nu) = \dfrac{8 \pi h \nu^3}{c^3 \left( 1 - e^{\dfrac{hc}{\lambda k_B T}} \right)
\end{equation}
...
On pourrait s'attendre à ce que le nombtre d'électrons extraits soit proportionnel à l'énergie reçue par le métal, donc à l'intensité lumineuse appliquée. Or, on constate expérimentalement qu'il existe un effet de seuil en longueur d'onde :
\begin{itemize}
	\item En-dessous d'une fréquence critique $\nu_c = c/\lambda_c$, aucun électron n'est extrait ;
	\item Au contraire, lorsque $\nu \ge \nu_c$, l'effet photoélectrique se manifeste et le nombre d'électrons extraits n'augmente pas si la fréquence est augmentée.
\end{itemize}
...
Depuis, des expériences similaires ont été reproduites avec des fentes d'Young : on observe alors des franges apparaître sur un écran où l'on mesure les impacts d'électrons (Tonomura et al, 1989).

Comment interpréter ce résultat ? On pourrait penser que les interférences observées sont liées à des électrons qui ont traversé les deux fentes d'Young simultanément, puis ont interagi. Mais ce n'est pas le cas, car les électrons sont envoyés un par un dans le dispositif. Ainsi, \textbf{les interférences se produisent même lorsqu'on considère le passage d'un électron unique} : l'électron est sensible à la présence des deux fentes à la fois. L'électron ne peut donc pas être décrit pas un point matériel.
...
\begin{remark}
	L'unité de la fonction d'onde dépend de la simension du problème. On la retrouve en remarquant que les probabilités sont toujours sans unité. Dans le cas d'un problème unidimensionnel, la fonction d'onde s'exprime en m$^{-1/2}$. 
\end{remark}
...
\begin{remark}
	...
	\begin{equation}
        ...
        \end{equation}
	Attention ! Ce n'est pas une équation de diffusion à cause de la présence de $i$, qui implique que l'équation n'est pas irréversible, par exemple.
\end{remark}
...
\begin{definition}[Puits de potentiel infini]
	...
	\begin{align}
		V(x) &= 0 \quad \mathrm{pour} 0 \le x \le L \\
...
La densité de probabilité de présence $|\varphi(x)|^2$ dans les premiers états stationnaires est représentée sur la figure XXX. On peut y constater que :
\begin{itemize}
...
\end{equation}
On peut remarquer que, dans le cas du puits de potentiel infini, la probabilité que la particule soit sur le segment $x \in [0,L]$ est égale à 1, dans laquelle $V(x) = 0$. Ainsi, l'énergie potentielle de la particule est nulle et $E_1$ correspond à son énergie cinétique. Ce résultat est en contradiction avec la mécanique classique qui prévoit qu'à l'équilibre, la particule est immobile, l'énergie cinétique est nulle, et la position correspond au minimum d'énergie potentielle. Autrement dit, la particule présente une énergie dite de confinement.
...
Considérons désormais un puits de potentiel de forme quelconque, typiquement on peut étudier l'atome d'hydrogène dont le potentiel est de la forme :
\begin{equation}
...
\end{proposition}
\begin{demonstration}
	\textbf{XXXXXXXX COMPLETER} Eventuellement mettre la démo ici même si hors programme ? Mais pas tres intéressante.
\end{demonstration}
