...
Dans les chapitres précédents, nous nous sommes uniquement intéressés à la description des états d'équilibre initiaux et finaux au cours d'une transformation thermodynamique. Mais il y a plusieurs aspects que nous n'avons pas étudiés :
\begin{itemize}
...
Ainsi, on choisit une échelle mésoscopique intermédiaire $\ell$ grande devant la taille d'une modélcule ou d'un atome et devant son libre parcours moyen, et petite devant la taille macroscopique du système, et on suppose que les temps caractéristiques d'évolution de la température sont suffisamment lents pour que l'on puisse considérer que des petits volume de taille $\dd \tau \sim \ell^3$ soient à tout instant à l'équilibre thermodynamique.

\subsection{Vecteur densité de courant thermique}

Puisque le système thermodynamique que l'on considère n'est pas à l'équilibre, il est le siège de transferts thermiques : de l'énergie thermique est transportée au sein du système. Ce transport se fait généralement depuis les régions les plus chaudes vers les plus froides (c'est le sens qui contribue à faire augmenter l'entropie). Pour le modéliser, on utilise le vecteur densité de courant thermique :
\begin{definition}[Vecteur densité de courant thermique]
	On appelle vecteur densité de courant thermique $\vv{j_th}$ le vecteur tel que la puissance thermique traversant une surface élémentaire orientée $\vv{\dd S}$ soit donnée par :
	\begin{equation}
...
\begin{proposition}[Flux thermique à travers une surface]
	...
	\begin{equation}
		\mathcal{P}_{th} = \Phi = \iint_\Sigma \vv{j_th} \cdot \vv{\dd S}
	\end{equation}
\end{proposition}

\begin{remark}
	On utilisera indifféremment les termes \textit{flux thermique} et \textit{puissance thermique}, puisque l'on vient de montrer qu'ils sont équivalents.
\end{remark}

\subsection{Modélisation des transferts thermiques}

Dans la nature, il existe trois types de transferts thermiques :
...
Le rayonnement d'un corps se fait en général dans le domaine de longueur d'onde de l'infrarouge thermique, autour de 10\,µm, sauf pour les corps très chauds comme le soleil ou une flamme, pour lesquels un rayonnement visible peut être émis.

\begin{remark}
	Ces transferts thermiques sont quantifiés par la loi de Stefan-Boltzmann (hors programme), qui correspond à l'intégrale spectrale de la loi de Planck. Ce sont eux qui sont responsables de l'effet de serre et permettent d'expliquer le réchauffement climatique d'origine humaine.
\end{remark}
...
\begin{capex}
	Interpréter la loi de Fourier à partir de cas limites (gradient faible ou fort) et commenter le signe du vecteur densité de courant thermique.
\end{capex}
...
La convection joue aussi un rôle à l'interface entre un solide et un fluide : si le fluide est plus froid que le solide, il va extraire de l'énergie thermique du solide, et inversement. On a continuité du flux thermique à une interface :
\begin{proposition}
...
\begin{capex}
	En déduire l'expression du flux thermique $\Phi$ qui traverse une interface de surface $S$.
\end{capex}
