...
La loi des maills peut également s'obtenir à partir de l'ARQS. Considérons un circuit électrique fermé, correspondant à un contour $\Gamma$. Par définition, la force électromotrice $e$ le long de ce circuit est la circulation du champ électrique le long de ce contour, qui est définie comme :
\begin{equation}
	e = \oint_\Gamma \vv E \cdot \vv{\dd \ell}
\end{equation}
Nous appliquons la loi de Faraday à ce circuit, et négligons la dérivée temporelle dans le cadre de l'ARQS :
\begin{equation}
	e = - \frac{\dd \Phi}{\dd t} = 0
\end{equation}
où $\Phi$ est le flux du champ magnétique à travers le circuit. Nous en concluons directement que $e=0$, c'est-à-dire que la somme des tensions aux bornes de chaque dipôle s'annule, ce qui correspond à la loi des mailles.
...
\begin{capex}
	Etablir l'équation de propagation du champ électromagnétique dans un conducteur de conductivité $\gamma$. En déduire la relation de dispersion d'une onde plane progressive harmonique.  
\end{capex}
...
Nous avons vu que dans les conducteurs, les ondes électromagnétiques ne peuvent pas pénétrer avec une profondeur trop importante. Il est possible de simplifier ce modèle en considérant le cas limite du conducteur parfait : dans ce cas, la conductivité tend vers l'infini, si bien que l'épaisseur de peau tend vers zéro :
...
\begin{demonstration}
	...
	Commençons par étudier la relation de passage pour la composante normale du champ électrique. On définit un volume élémentaire $\dd V$ constitué de deux éléments de surface $\dd S$ situés juste de part et d'autre de l'interface au niveau du point $M$. On applique le théorème de Gauss à ce volume, le flux du champ électrique sortant du volume est :
	\begin{equation}
	...
	\begin{equation}
		\oint_{\dd \Gamma} \vv B \cdot \vv{\dd \ell} = \mu_0 I_{\rm enlace} + \frac{1}{c^2} \frac{\dd}{\dd t} \iint_{\dd S} \vv E \cdot \vv{\d S}
	\end{equation}
...
\begin{capex}
	Déterminer l'expression de l'onde réfléchie en fonction de l'onde réfléchie à partir des relations de passage. Quelle est le coefficient de réflexion en énergie ? En déduire que la puissance dissipée dans le conducteur parfait est nulle.
\end{capex}
