...
\textbf{Attention !} Dans le cas où la demi-équation électronique fait intervenir d'autres espèces (par exemple de l'eau, des ions hydrohyde ou oxonium) il convient de toujours équilibrer la demi-équation électronique avec des protons. En effet, le potentiel dépend du pH de la solution et les potentiels standard sont par convention tabulés à $pH = 0$ (il est ensuite possible de calculer le potentiel standard pour d'autres valeurs de pH à partir de la formule de Nernst).
...
\begin{capex}
	On étudie le fer, qui forme les composés suivants : $\ce{Fe(s), Fe^{3+}(aq), Fe^{2+}(aq), Fe(OH)2(s), Fe(OH)3(s)}$. Etablir le diagramme de situation du fer. 
\end{capex}
...
\textbf{Attention !} Le degré de protection dépend de la structure microscopique du solide formé. Dans le cas du fer, le composé $\ce{Fe(OH)3(s)}$ a une structure poreuse, si bien qu'il ne protège pas le métal de la solution. La réaction d'oxydation continue à se produire de plus en plus profond dans le solide jusqu'à l'attaquer complètement : c'est la rouille.
