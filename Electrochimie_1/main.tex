...
\begin{capex}
	A l'aide d'un bilan de quantité de matière, relier la durée $\Delta t$ de l'électrolyse au courant imposé pour réaliser l'électrolyse, noté $i$, et à la masse de dihydrogène théorique produite par électrolyse $m_{\ce{H2}, \rm th}$.
\end{capex}
...
\begin{encart}[Les batteries lithium-ion]
	La plupart des batteries présentes aujourd'hui dans les appareils électroniques sont des batteries lithium-ion. Elles sont constituées de deux électrodes en graphite $\ce{LiC6}$ et en oxyde de coubalt $\ce{LiCoO2}$, qui sont enrichis en lithium. Les électrodes sont séparées par un électrolyte qui conduit les ions lithium $\ce{Li^+}$ et les deux demi-équations se produisant lors de la phase de recharge de la batterie sont :
	\begin{align}
		&\ce{LiCoO2 = CoO2 + Li^+ + e^-} \\
		&\ce{C6 + Li^+ + e^- = LiC6}
	\end{align}
...
\begin{figure}[htbp]
	\centering \includegraphics[width=\linewidth]{li_ion.jpg}
