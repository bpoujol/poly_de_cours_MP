...
La cause principale de ces mouvements, d'après la mécanique newtonienne, est la présence d'actions mécaniques. Il en existe pulsieurs types :
...
\begin{theorem}[Principe fondamental de la dynamique]
...
\begin{theorem}[Théorème du moment cinétique]
    Soit $M$ un point matériel de masse $m$ soumis à un ensemble de forces $\vv F_{ext}$. Dans un référentiel galiléen dans lequel le point $O$ est fixe, la variation du moment cinétique $\vv{L_O}$ du point $M$ par rapport au point $O$ vaut :
    \begin{equation}
...
\begin{theorem}[Loi de la gravitation universelle]
    ...
    \begin{equation}
        \vv F_{1 \rightarrow 2} = - G \dfrac{m_1 m_2}{\left|\left|\vv{M_1 M_2}\right|\right|^2} \dfrac{\vv{M_1 M_2}}{\left|\left|\vv{M_1 M_2}\right|\right|}
    \end{equation}
...
\begin{example}
    ...
    \end{align}
\end{example}
On constate que le travail de la force gravitationnelle ne dépend que de la position initiale et de la position finale de la particule, données par $r_1$ et $r_2$: il est indépendant du chemin suivi pour relier ces deux positions. On en déduit que la force gravitationnelle est une force conservative :
...
Nous cherchons maintenant à relier le potentiel gravitationnel et le champ gravitationnel par une relation directe, sans faire intervenir d'intégrale. Pour cela, nous pouvons remarquer que le travail élémentaire de la force électrique le long d'un chemin $\mathrm{d} \vv l$ peut s'écrire d'une part:
\begin{equation}
...
Maintenant que nous avons introduit l'opérateur gradient, nous obtenons un lien direct entre champ gravitationnel et potentiel gravitationnel :
...
\begin{demonstration}
	Le travail élémentaire de la force gravitationnelle le long d'un élément de chemin $\vv{\mathrm{d} \ell}$ peut s'écrire d'une part:
\begin{equation}
...
\end{equation}
	par propriété de l'opérateur gradient. Par identication, on en déduit:
\begin{equation}
